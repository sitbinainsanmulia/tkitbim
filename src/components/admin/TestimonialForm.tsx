...
import { createTestimonial, updateTestimonial } from '@/app/actions/admin';
...
export function TestimonialForm({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // Jika tidak ada data awal (tambah baru via admin), otomatis aktif
    if (!initialData) {
      ...
      <div className="bg-surface w-full max-w-lg rounded-3xl p-6 shadow-2xl relative max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1">
            <label className="text-sm font-bold text-on-surface">Nama Orang Tua/Wali *</label>
            <input required type="text" name="parent_name" defaultValue={initialData?.parent_name} className="w-full p-3 rounded-xl border border-outline-variant/50 focus:outline-primary" placeholder="Contoh: Bpk. Budi Santoso" />
          </div>

          <div className="space-y-1">
            <label className="text-sm font-bold text-on-surface">Nama Anak (Opsional)</label>
            <input type="text" name="child_name" defaultValue={initialData?.child_name} className="w-full p-3 rounded-xl border border-outline-variant/50 focus:outline-primary" placeholder="Contoh: Ahmad (TK B)" />
          </div>

          <div className="space-y-1">
            <label className="text-sm font-bold text-on-surface block mb-2">Rating ({rating} Bintang)</label>
            <div className="flex gap-2">
              {[1, 2, 3, 4, 5].map((star) => (
                ...
                >
                  <span className={`material-symbols-outlined text-4xl ${star <= rating ? 'text-yellow-400 drop-shadow-sm' : 'text-outline-variant/30'}`} style={{ fontVariationSettings: "'FILL' 1" }}>
                    star
                  </span>
                </button>
              ))}
          ...
          <div className="space-y-1">
            <label className="text-sm font-bold text-on-surface">Isi Ulasan *</label>
            <textarea required name="content" defaultValue={initialData?.content} rows={4} className="w-full p-3 rounded-xl border border-outline-variant/50 focus:outline-primary resize-none" placeholder="Tuliskan ulasan, kesan, atau pesan..." />
          </div>
